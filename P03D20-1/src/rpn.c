    ...
    for (int i = 0; i < n; i++) {
        if (root[i].type == 1) {
            ...
            if (h != 13) {
                ...
                if (h == 11) {  // Операция деления
                    struct node *top_node = peek(&execution);
                    if (top_node == NULL || top_node->data.type != 0) {
                        return NAN;  // Возврат NaN в случае ошибки стека или неправильного типа токена
                    }

                    double second = top_node->data.operand;
                    if (second == 0) {
                        return NAN;  // Возврат NaN в случае попытки деления на ноль
                    }

                    free(top_node);  // Освобождение памяти занятой вершины
                }
                func[h - 1](&execution);
            } else {
                replace(&execution, x);
